...
# Add a function “deal” to Deck that takes two parameters representing the number of hands and the number of cards per hand and returns a list of Hands. If the number of cards per hand is set to -1, all of the cards should be dealt, even if this results in an uneven number of cards per hand. Write tests to verify that this works. 	
	def deal(self,num_hand,num_card_per):
                            lst_hands=[]
                            if num_card_per==-1:
                                   for num in range(num_hand):
                                               dealhand=Hand()
                                               lst_hands.append(dealhand)
                            return lst_hands
...
class Hand():
      ...
      def remove_pairs(self):
          ...
          for cd in self.cards:
              if cd.rank not in testl:
                      testl.append(cd.rank)
                      pairoff.append(cd)
              else:
                      for paircd in pairoff:
                          if paircd.rank==cd.rank:
                             pairoff.remove(paircd)
          self.cards=pairoff
          return self.cards
...
#Add a function “remove_pairs” to Hand that looks for pairs of cards in a hand and removes them. Note that if there are three of a kind, only two should be removed (it doesn’t matter which two). Write tests to verify that this works.       
      def test_pair(self):
          thand=Hand()
          thand.remove_pairs() 
          self.assertEqual(len(thand.cards),1)
          self.assertEqual(thand.cards[0].rank,2)    

      def test_Deal(self):
          onedeal=Deck().deal(1,2)
          newdeal=Deck().deal(5,-1)
          self.assertEqual(len(newdeal),5)
          self.assertEqual(len(onedeal),0)
